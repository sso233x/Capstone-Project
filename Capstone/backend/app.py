...
class GymClass(db.Model):
    ...
    attendees = db.relationship('Attendance', backref='class_ref', lazy=True)
...
def login():
    ...
    if user and bcrypt.check_password_hash(user.password, data['password']):
        # Include 'name', 'email', and 'role' in the identity
        token = create_access_token(identity={
            'id': user.id,
            'name': user.name,   # Add the name field here
            'email': user.email, # Add the email field here
            'role': user.role    # Keep the role field as it is
        })
